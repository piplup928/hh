...
class TextEmbedding(nn.Module):
    def __init__(self, text_num_embeds, text_dim, mask_padding=True, conv_layers=0, conv_mult=2):
        super().__init__()
        self.text_embed = nn.Embedding(text_num_embeds, text_dim)  # use 0 as filler token

        self.mask_padding = mask_padding  # mask filler and batch padding tokens or not

        if conv_layers > 0:
            self.extra_modeling = True
            self.precompute_max_pos = 2000  # ~44s of 24khz audio
            self.register_buffer("freqs_cis", precompute_freqs_cis(text_dim, self.precompute_max_pos), persistent=False)
            # PLATFORM PATCH: device placement handled by the caller's
            # model.to(device); hardcoded .to('cuda') breaks cpu/mps machines.
...
class DiT(nn.Module):
    def __init__(self, config: ModelConfig):
        ...
        self.text_embed = TextEmbedding(text_num_embeds=num_text_embedding, text_dim=text_dim, mask_padding=text_mask_padding, conv_layers=conv_layers)
